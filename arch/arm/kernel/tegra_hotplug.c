...
static void __ref decide_hotplug_func(struct work_struct *work)
{
	...
	if (unlikely(hot_data->online_cpus == 1))
		queue_delayed_work(pm_wq, &decide_hotplug, msecs_to_jiffies(hot_data->hotplug_sampling * HZ));
...
static struct attribute *grouper_hotplug_attributes[] = 
{
...
{
	.attrs  = grouper_hotplug_attributes,
};
...
int __init grouper_hotplug_init(void)
{
	int ret;
#ifdef grouper_DEBUG
	pr_info("[Hot-Plug]: grouper Hotplug driver started.\n");
#endif
